...
with open('urls_registration.txt', 'a') as f:
    for i in sorted_files[:959]:
        ...
        if i.endswith('.wav'):
            ...
            if result:
                ...
                value_in = f'/media/registration/{new_i}_registration.wav'
                # print(value_in)
                if value_in in string_val:
                    print('yes')
                    string_val = string_val.replace(f'/media/registration/{new_i}_registration.wav',f"http://192.168.1.251:8080/Registration Audio/{new_i}_registration.wav")
